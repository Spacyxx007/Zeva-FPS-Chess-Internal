...
#ifndef ZEROGUI_H
#define ZEROGUI_H
#include "../FPS Chess/SDK/Engine_classes.hpp"
#include "ZeroInput.h"

using FLinearColor = SDK::FLinearColor;
using FVector2D = SDK::FVector2D;
using UCanvas = SDK::UCanvas;

#ifndef crypt
#define crypt(str) str  // Simple passthrough if no encryption needed
#endif

namespace Functions
{
	inline SDK::UFont* NamesFont = nullptr;
}
#pragma warning(push)
...
namespace ZeroGUI
{
	namespace Colors
	{
		...
		inline FLinearColor Window_Background{ 0.009f, 0.009f, 0.009f, 1.0f };
		inline FLinearColor Window_Header{ 0.10f, 0.15f, 0.84f, 1.0f };

		inline FLinearColor Button_Idle{ 0.10f, 0.15f, 0.84f, 1.0f };
		inline FLinearColor Button_Hovered{ 0.15f, 0.20f, 0.89f, 1.0f };
		inline FLinearColor Button_Active{ 0.20f, 0.25f, 0.94f, 1.0f };

		inline FLinearColor Checkbox_Idle{ 0.17f, 0.16f, 0.23f, 1.0f };
		inline FLinearColor Checkbox_Hovered{ 0.22f, 0.30f, 0.72f, 1.0f };
		inline FLinearColor Checkbox_Enabled{ 0.20f, 0.25f, 0.94f, 1.0f };
		...
		inline FLinearColor Slider_Idle{ 0.17f, 0.16f, 0.23f, 1.0f };
		inline FLinearColor Slider_Hovered{ 0.17f, 0.16f, 0.23f, 1.0f };
		inline FLinearColor Slider_Progress{ 0.22f, 0.30f, 0.72f, 1.0f };
		inline FLinearColor Slider_Button{ 0.10f, 0.15f, 0.84f, 1.0f };
	...
	inline void TextLeft(char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		...
		canvas->K2_DrawText(Functions::NamesFont, SDK::FString(s2wc(name)), pos, FVector2D{ 0.97f, 0.97f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, false, true, true, Colors::Text_Outline);
	}
	inline void TextCenter(char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		int length = strlen(name) + 1;
		canvas->K2_DrawText(Functions::NamesFont, SDK::FString(s2wc(name)), pos, FVector2D{ 0.97f, 0.97f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, true, true, true, Colors::Text_Outline);
	}
	...
	inline void SliderFloat(char* name, float* value, float min, float max, const char* format = "%.0f")
	{
		elements_count++;
	...
	inline char* active_input_buffer = nullptr;
	inline int active_input = -1;
	inline bool input_already_pressed = false;

	inline bool InputText(char* name, char* buffer, int bufferSize)
	{
		elements_count++;

		FVector2D size = FVector2D{ 200, 25 };
		FVector2D padding = FVector2D{ 10, 10 };
		FVector2D pos = FVector2D{ menu_pos.X + padding.X + offset_x, menu_pos.Y + padding.Y + offset_y };
		...
		bool isHovered = MouseInZone(FVector2D{ pos.X, pos.Y }, size);
		bool isActive = (active_input == elements_count && active_input_buffer == buffer);
		...
		{
			FVector2D labelPos = FVector2D{ pos.X, pos.Y - 20 };
			TextLeft(name, labelPos, Colors::Text, false);
		}

		// Text content
		FVector2D textPos = FVector2D{ pos.X + 5, pos.Y + size.Y / 2 };
		if (strlen(buffer) > 0)
		{
		...
		{
			TextLeft((char*)"...", textPos, FLinearColor{ 0.5f, 0.5f, 0.5f, 1.0f }, false);
		}

		// Cursor si actif
		if (isActive)
		{
			float cursorX = pos.X + 5 + (strlen(buffer) * 7); // Approximation de la largeur du texte
			Draw_Line(FVector2D{ cursorX, pos.Y + 5 }, FVector2D{ cursorX, pos.Y + size.Y - 5 }, 2.0f, Colors::Text);
		}

		// Gestion du clic
		if (isHovered && Input::IsMouseClicked(0, elements_count, false))
		{
			active_input = elements_count;
			active_input_buffer = buffer;
			input_already_pressed = true;
		}
		else if (!isHovered && Input::IsMouseClicked(0, elements_count, false))
		{
			if (active_input == elements_count)
			{
				active_input = -1;
				active_input_buffer = nullptr;
			}
		}

		// Input clavier
		if (isActive && !input_already_pressed)
		{
			for (int key = 0x08; key <= 0xFE; key++)
			{
				if (GetAsyncKeyState(key) & 1)
				{
					...
					if (key == VK_BACK && len > 0) // Backspace
					{
						buffer[len - 1] = '\0';
					}
					...
					else if (key == VK_SPACE && len < bufferSize - 1)
					{
						buffer[len] = ' ';
						buffer[len + 1] = '\0';
					}
					else if (len < bufferSize - 1)
					{
						...
						if (key >= 0x30 && key <= 0x39) // 0-9
						{
							if (shift)
							{
								...
								buffer[len] = shiftNumbers[key - 0x30];
							}
							else
							{
								buffer[len] = key;
						...
						{
							buffer[len] = shift ? key : key + 32;
							buffer[len + 1] = '\0';
						}
						else if (key >= VK_NUMPAD0 && key <= VK_NUMPAD9)
						{
							buffer[len] = (key - VK_NUMPAD0) + '0';
							buffer[len + 1] = '\0';
						}
